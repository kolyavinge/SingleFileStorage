...
namespace StarterApp
{
    class Program
    {
        ...
        private static void Sequence()
        {
            ...
            var storage = StorageBuilder.Create("image12.storage");

            // image1.jpg write
            storage.CreateRecord("image1.jpg");
            using (var stream = storage.OpenRecord("image1.jpg", RecordAccess.ReadWrite))
            {
                stream.Write(image1FileContent, 0, image1FileContent.Length);
            }
            // image2.jpg write
            storage.CreateRecord("image2.jpg");
            using (var stream = storage.OpenRecord("image2.jpg", RecordAccess.ReadWrite))
            {
                stream.Write(image2FileContent, 0, image2FileContent.Length);
            }
            // image1.jpg read
            var image1RecordReadContent = new byte[image1FileContent.Length];
            using (var stream = storage.OpenRecord("image1.jpg", RecordAccess.Read))
            {
            ...
            using (var stream = storage.OpenRecord("image2.jpg", RecordAccess.Read))
            {
